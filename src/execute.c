...
/*   Updated: 2025/06/20 10:53:38 by iboubkri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int execute_command(t_cmd *cmd, t_cmd *builtins, char **arguments, int *streams)
{
	...
	if (cmd->func && streams[UNUSED] == -1)
		return (s_data.exit_status = cmd->func(arguments) << 8, free(cmd->path),
				0);
	pid = fork();
	if (pid < 0)
	...
	execve(cmd->path, arguments, s_data.environs);
	perror(arguments[0]);
...
int execute_pipeline(t_tree *tree, int *streams)
{
	...
	if (open_redirections(tree->s_command.redirections, streams) == -1)
		return (s_data.exit_status = 1 << 8, 1);
	cmd = (t_cmd){ft_strdup(tree->s_command.arguments[0]), NULL};
	execute_command(&cmd, (t_cmd[]){{"echo", ft_echo}, {"cd", ft_cd}, {"pwd", ft_pwd}, {"export", ft_export}, {"unset", ft_unset}, {"env", ft_env}, {"exit", ft_exit}, {NULL, NULL}}, tree->s_command.arguments, streams);
	return (0);
}
