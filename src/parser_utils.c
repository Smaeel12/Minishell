...
/*   Updated: 2025/06/14 13:04:35 by iboubkri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		char *line, size_t len)
{
	...
	if (((state == DQTS || state == SQTS || state == WORD) && (cstate == WORD
				|| cstate == DQTS || cstate == SQTS)))
		return (0);
	if (state != SCAN)
	{
...
char	*expand_line(t_token *token)
{
	...
	while (token->value[i])
	{
		...
		while (token->value[++i] && !(token->value[i - 1] == '$'
			...
			;
		key = &token->value[i];
		while (*key && token->value[++i] && (ft_isalnum(token->value[i])
				|| token->value[i] == '_'))
			;
		key = ft_substr(key, 0, (&token->value[i] - key));
		result = create_line((char *[]){result, ft_substr(token->value, start, i
