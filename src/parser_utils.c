...
/*   Updated: 2025/07/03 04:38:08 by iboubkri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int expand_line(char **result, char *line, bool modes[], size_t max)
{
	...
	while (line[i++])
	{
		...
		while ((ft_isalnum(line[i]) || line[i] == '_'))
			i++;
		i += (line[i] && line[i] == '?');
		idx += append_string(&result[idx], ft_substr(line, beg, end - beg - (i > end)), max, false);
		idx += append_string(&result[idx], get_env(ft_substr(line, end, i - end)), max, (st != SQTS && st != DQTS && max > 2));
